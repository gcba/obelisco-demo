...
var NavItemComponentSlider = function (props) {
    var name = props.name, id = props.id, children = props.children, disabled = props.disabled, selected = props.selected, level = props.level, iconTabs = props.iconTabs, hasIcon = props.hasIcon;
    var isActive = selected === id;
    var linkClassName = classnames_1.default('nav-link', {
        active: isActive,
        disabled: disabled
    });
    return (react_1.default.createElement("li", { className: "nav-item", role: "presentation" },
        react_1.default.createElement("button", { className: linkClassName, disabled: disabled, tabIndex: disabled ? -1 : undefined, "data-toggle": "tab", "data-target": "#panel-content-" + id, type: "button", role: "tab", "aria-controls": "panel-content-" + id, "aria-selected": isActive }, hasIcon && iconTabs ? (react_1.default.createElement("div", { className: "nav-icon", dangerouslySetInnerHTML: { __html: name + " " + iconTabs } })) : (react_1.default.createElement(react_1.default.Fragment, null, name))),
        children && (react_1.default.createElement("ul", { className: listClasses, role: "tabpanel", id: "panel-" + id }, children.map(function (item) { return (react_1.default.createElement(_1.NavItemComponent, __assign({}, item, { key: item.id, selected: selected, level: level }))); })))));
};
var listClasses = 'nav nav-pills tabs';
exports.NavTabsSlider = function (props) {
    var items = props.items, navSize = props.navSize, hasIcon = props.hasIcon, classUl = props.classUl, _a = props.isWithButton, isWithButton = _a === void 0 ? true : _a, selected = props.selected, idPrefix = props.idPrefix;
    var activeTabId = react_1.useState(selected || '')[0]; // Valor predeterminado de cadena vacía si selected es null o undefined
    ...
    return (react_1.default.createElement(react_1.default.Fragment, null,
        react_1.default.createElement("nav", { className: "tabs-slider", "aria-label": props.label },
            react_1.default.createElement("ul", { className: mainListClasses, ref: tabsBoxRef, onMouseDown: handleMouseDown, onMouseUp: handleMouseUp, onMouseMove: handleMouseMove, onTouchStart: handleTouchStart, onTouchMove: handleTouchMove, onTouchEnd: handleTouchEnd, onMouseLeave: handleMouseLeave, id: idPrefix + "-myTab", role: "tablist" }, items.map(function (item) { return (react_1.default.createElement(NavItemComponentSlider, __assign({}, item, { key: item.id, hasIcon: hasIcon, type: item.type, selected: activeTabId, level: 0 }))); })),
            isWithButton && (react_1.default.createElement("div", { className: "icons-container" },
                react_1.default.createElement("button", { className: "right-left icon", onClick: handleIconClick, id: "left" },
                    react_1.default.createElement("span", { className: "material-icons-round" }, "arrow_back_ios")),
                react_1.default.createElement("button", { className: "right-left icon", onClick: handleIconClick, id: "right" },
                    react_1.default.createElement("span", { className: "material-icons-round" }, "arrow_forward_ios"))))),
        react_1.default.createElement("div", { className: "tab-content", id: idPrefix + "-myTabContent" }, items.map(function (item) { return (react_1.default.createElement("div", { key: item.id, className: classnames_1.default('tab-pane', {
                fade: true,
                show: item.id === activeTabId,
                active: item.id === activeTabId
            }), id: "panel-content-" + item.id, role: "tabpanel", "aria-label": item.id + "-tab" }, item.content)); }))));
};
